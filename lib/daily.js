...
const seedLinks = async () => {
  ...
  for (let i = dailyGameID_init + 1; i <= dailyGameID_init + 100; i++) {
    ...
    do
    {
      ...
      art1_pop = await spot.getArtist(start_id).popularity;
      art2_pop = await spot.getArtist(end_id).popularity;
    } while(par == 1 && await art2_pop > (art1_pop / 2))
...
const seedLinksAlbums = async () => {
  ...
  for (let i = dailyGameID_init + 1; i <= dailyGameID_init + 100; i++) {
    ...
    do
    {
      var album_ids = await logic.getValidAlbumStartEnd(genre.name);
      new Promise((resolve) => setTimeout(resolve, 2000))
      var start_id = await album_ids.id1;
      var end_id = await album_ids.id2;
      par = await album_ids.par;
    } while(par == 1)
