...
export const getValidArtistStartEnd = async (genre, specific_mode) => {
  ...
  if(artist_queue.length < 1)
    return {id1: startID, id2: curr_artist, par: checked_artists.length}; // if startID = curr_artist then the artist has no connections, have it restart.
  else
...
export const getValidAlbumStartEnd = async (genre) => {
  // implement random DFS, randomize link 4-7 albums long.
  // initialize stacks
...
//var m = await getValidAlbumStartEnd("alternative", false);
//console.log(m);

//var n = await verifyAlbumPath(m[0], m[1]);
//console.log(n);

export const getArtistArtist = async () => {
  ...
  //var date = new Date();
  var selected = gamesID[randomInt(0, gamesID.length)];
  var toReturn = {
    id1: selected.start_artist_id,
    id2: selected.end_artist_id,
    par: selected.challenge_link_length
  };
  return toReturn;
}

export const getAlbumAlbum = async () => {
  ...
  var selected = gamesID[randomInt(0, gamesID.length)];
  var toReturn = {
    id1: selected.start_album_id,
    id2: selected.end_album_id,
    par: (selected.challenge_link_length + 1)
  };
  return toReturn;
}
