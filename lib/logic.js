...
export const getSpecificValidArtistStartEnd = async (startID) => { // startID asked for rather than randomized
  // implement random DFS, randomize link 3-6 artists long.
  // initialize stacks
  let artist_queue = [];
  let checked_artists = [];
...
export const getValidAlbumStartEnd = async (genre) => {
  ...
  var se = await getSpecificValidArtistStartEnd(startArtistID);
  var endAlbums = await spot.getArtistAlbums(se[1]);
  var endID = await endAlbums[randomInt(0,endAlbums.length)].id;
  return await [startID, endID, se[2] + 1];
}

var m = await getValidAlbumStartEnd("alternative")
console.log(m);
