...
//Generates and returns a random genre based on the json
export const getRandomGenre = async () => {
  try {
    const jsonString = await fs.readFileSync('./genre-seeds.json', 'utf-8');
    const data = JSON.parse(jsonString);
    return {name: data.genres[Math.floor(Math.random() * data.genres.length)]};
  } catch (err) {
    throw new Error("genre-seed.json has issues: " + err);
  }
};
